...
export default function RegisterPage() {
  ...
  const [done, setDone] = useState<{ verifyLink: string | null } | null>(null);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await api<{ verifyLink: string | null }>('/api/auth/register', { method: 'POST', body: form });
      setDone(res);
    } catch (err: any) {
        ...
        <div className="card space-y-4 p-8 text-center">
          ...
          {done.verifyLink ? (
            <p className="text-sm text-slate-600">
              We&apos;ve emailed a verification link to activate your account. In this dev setup you can{' '}
              <a href={done.verifyLink} className="text-brand-600 underline">
                verify now
              </a>
              .
            </p>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className="input" value={form.matricNumber} onChange={(e) => set('matricNumber', e.target.value)} required />
            </div>
          </div>
          <div>
